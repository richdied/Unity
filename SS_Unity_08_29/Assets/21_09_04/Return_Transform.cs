...
public class Return_Transform : MonoBehaviour
{
    ...
    //      [ new Vector(float x, float y, float z) ] 형태를 가지고 있다.

    //[3-1] Vector타입 변수를 선언하여 객체의 위치에 할당하기.
    public Vector3 My_Pos;

    [Header("입력 및 시간 관련 변수")]
    public int Count = 0; //플레이어의 현재 포지션 카운트
    ...
    void Update()
    {
        ...
        //  {//좌클릭이 누르고있는 동안 true를 반환하겠다.
        //    print("Mouse_Stay");
        // }
        // if (Input.GetMouseButtonUp(0))
        // {//좌클릭 입력이 끝나는 순간 true를 반환겠다.
        //   print("Mouse_Up");
        // }

        //transform 컴포넌트 기초 활용예제.

        //[1] 마우스 좌클릭을 누를 때마다 화면이 각 모서리를 
        ...
        if (Is_Time == true)
        {
            ...
            {
                Count++;
                gameObject.transform.position = new Vector3(8, 4, 0);
            }
            else if(Count == 1)
            {
                Count++;
